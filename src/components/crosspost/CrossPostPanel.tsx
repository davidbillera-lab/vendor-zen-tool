...
export function CrossPostPanel({ listing, images, projectId, listingId: initialListingId, preselectedPlatform }: CrossPostPanelProps) {
  const [rows, setRows] = useState<Record<string, PlatformRowState>>(() =>
    Object.fromEntries(
      PLATFORM_ADAPTERS.map(p => [p.id, {
        checked: preselectedPlatform === p.id, status: 'idle', previewOpen: false,
  ...
  // Auto-check preselected platform when prop changes
  useEffect(() => {
    if (preselectedPlatform) {
      setRows(prev => ({
        ...prev,
        [preselectedPlatform]: { ...prev[preselectedPlatform], checked: true },
      }));
    }
  }, [preselectedPlatform]);
  ...
  async function handleCrossPost() {
    ...
    if (succeeded.length > 0) {
      toast({ title: 'Cross-post dispatched', description: `Queued for: ${succeeded.join(', ')}` });
    }
    if (failed.length > 0) {
      toast({ title: 'Some platforms failed', description: failed.join(', '), variant: 'destructive' });
    }
  }
