...
/**
 * processLot(page, lot, imagePaths, options)
 *
 ...
 *
 * Options:
 *   descriptionsOnly {boolean} — skip title fill and image upload; only write
 *                                the description and save. Use this to patch
 *                                lots that already have titles/images uploaded.
 *
 * Returns the URL of the next page.
 */
async function processLot(page, lot, imagePaths, options = {}) {
  const { descriptionsOnly = false } = options;
  ...
  if (descriptionsOnly) {
    ...
    // Title and images are already on DOA — only fill the description and save.
    if (lot.description && lot.description.trim()) {
      await fillDescriptionOnly(page, lot);
    } else {
      log.warn(`  Lot #${lot.lot_number}: no description in CSV — skipping (nothing to patch)`);
    }
  } else {
    // ── Normal mode ────────────────────────────────────────────────────────
  ...
  // Save and advance (always)
  const nextUrl = await saveLot(page, lot.lot_number);

  return nextUrl;
}

// ── Fill description only (repair pass) ──────────────────────────────────────
 ...
 *   descriptionsOnly  {boolean}  Skip title/image steps; only fill description
 *
...
export async function runDoaAgent(lots, options = {}, callbacks = {}) {
  const { firstLotUrl: passedFirstLotUrl, descriptionsOnly = false } = options;
  ...
  try {
    ...
    for (let i = 0; i < lots.length; i++) {
      ...
      try {
        ...
        const nextUrl = await processLot(page, lot, imagePaths, { descriptionsOnly });
