...
serve(async (req) => {
  ...
  try {
    ...
    // ════════════════════════════════════════════════════════════════════
    // EBAY ONLY: Second-pass verification with GPT-5-mini
    // Cross-checks the primary identification for accuracy
    // ════════════════════════════════════════════════════════════════════
    if (platform === 'ebay' && parsedListing) {
      console.log('Starting eBay verification pass (GPT-5-mini)...');
      try {
        ...
        verifyContent.push({
          type: "text",
          text: `You are a specialist item authenticator and identifier for eBay listings. A first-pass AI generated the listing below. Your job is to VERIFY and CORRECT any misidentifications.

CAREFULLY study the image(s) and compare against the generated listing. Focus especially on:
...
${additionalContext ? `Seller context: ${additionalContext}` : ''}

FIRST-PASS LISTING TO VERIFY:
Title: ${parsedListing.title}
Category: ${parsedListing.category} (ID: ${parsedListing.categoryId})
Price: $${parsedListing.price}
Condition: ${parsedListing.condition}
Item Specifics: ${JSON.stringify(parsedListing.itemSpecifics || {})}
Description (first 200 chars): ${(parsedListing.description || '').substring(0, 200)}...

RESPOND WITH VALID JSON ONLY — no markdown, no explanation:
{
  "verified": true/false (false if you found errors),
  "corrections": {
    "title": "corrected title or null if original is fine",
    "categoryId": corrected_number_or_null,
    "category": "corrected category name or null",
    "price": corrected_number_or_null,
    "condition": "corrected or null",
    "description": "fully rewritten description if item was misidentified, otherwise null",
    "itemSpecifics": { corrected/additional specifics } or null
  },
  "confidence": "high/medium/low",
  "notes": "brief explanation of what was wrong or confirmed"
}`
        });
        ...
        if (verifyResponse.ok) {
          ...
          try {
            const verifyMatch = verifyText.match(/```(?:json)?\s*([\s\S]*?)```/) || [null, verifyText];
            const verification = JSON.parse(verifyMatch[1].trim());
            
            console.log(`Verification result: verified=${verification.verified}, confidence=${verification.confidence}, notes=${verification.notes}`);

            // Apply corrections if the verifier found errors
            if (!verification.verified && verification.corrections) {
              const c = verification.corrections;
              if (c.title) {
                console.log(`Title corrected: "${parsedListing.title}" → "${c.title}"`);
                parsedListing.title = c.title;
              }
              if (c.categoryId) {
                console.log(`Category corrected: ${parsedListing.categoryId} → ${c.categoryId}`);
                parsedListing.categoryId = c.categoryId;
              }
              if (c.category) parsedListing.category = c.category;
              if (c.price) parsedListing.price = c.price;
              if (c.condition) parsedListing.condition = c.condition;
              if (c.description) parsedListing.description = c.description;
              if (c.itemSpecifics) {
                parsedListing.itemSpecifics = {
                  ...(parsedListing.itemSpecifics || {}),
                  ...c.itemSpecifics
                };
              }
            ...
            console.warn('Verification parse failed, using primary result:', verifyParseErr);
          }
        } else {
          const errText = await verifyResponse.text();
          console.warn(`Verification call failed (${verifyResponse.status}), using primary result: ${errText}`);
        }
      } catch (verifyErr) {
        console.warn('Verification pass error, using primary result:', verifyErr);
      }
    }
