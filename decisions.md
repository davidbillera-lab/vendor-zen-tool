...
## 2026-08-21 — Guardrail audit found and closed the last gap: eBay's pre-publish QA agent had none

**Context (David):** asked for confirmation the measurement guardrail is truly global — specifically calling out DOA and eBay. Audited every AI touchpoint in VZT (grepped all 8 edge functions that call Anthropic, plus checked the frontend for any direct AI calls that would bypass the edge functions).

**Findings:**
- **DOA (Denver) — already fully covered, confirmed.** No separate publish-time AI step exists for DOA; `generate-listing`'s denver prompt, `generate-denver-listing`, and `reformat-listing`'s denver path (all already guardrailed) are the only AI touchpoints. `DenverLotEditor.tsx`/`DenverBatches.tsx` don't construct their own prompts.
- **eBay had a real, previously-missed gap: `ebay-publish`'s pre-publish QA agent** (`runPrePublishQA`) — a Haiku call that runs right before the Trading API publish, filling missing required item specifics from title/description/images with zero measurement guardrail. For large/furniture categories where eBay requires Item Length/Width/Height/Weight, this agent could "confidently determine" a guessed dimension from photos and inject it at the very last step before going live — bypassing every other guardrail in the pipeline. Fixed the same way as `enrich-ebay-batch`: added the MEASUREMENT RULE to its prompt, and applied the same `sanitizeMeasurementSpecifics()` deterministic filter to both of its return paths (category-override and pass-through) using the narrow whole-key regex that doesn't collide with electronics "Storage Capacity" or clothing/jewelry "Size"/"Ring Size".
- **Precedence wording gap:** the guardrail text said it outranks "master instructions" but never named the Hermes-loop **LEARNED LESSONS** block, which sits even earlier in the assembled prompt (`generate-listing`, `refine-listing`). If a distilled lesson ever generalized from a past correction that added a real measurement, it could have out-argued the guardrail on a technicality. Reworded both to explicitly outrank learned lessons too, not just business context/master prompt.

**Important build note:** `ebay-publish` was 356 lines different between `main` and the unmerged `chore/doa-agent-distribution` (JSON-fence-stripping fix, better leaf-category detection, aspect-length clamping — all real, wanted fixes, none flagged as unsafe). First drafted this fix against `main`'s stale copy by mistake; caught it before committing and re-based the fix onto the current branch instead, to avoid silently reverting those improvements on deploy. Confirms deployed-production state tracks ad hoc `supabase functions deploy` runs, not `main` — same pattern already established.

Deployed directly to Supabase (no merge needed for edge functions, same as the rest of this work); `main` merge remains scoped to the frontend-only fix already shipped.

---

## 2026-08-21 — Hermes loop audit: fixed the two confirmed gaps (DOA mis-tagging, missing refine-listing injection tracking)

**Context:** Requested audit of the Hermes self-improving loop confirmed it is genuinely live in production (356 corrections captured, 100% embedded, 940 lessons ever distilled, 15 active at the cap, most recent activity within hours) and that all three Hermes tables are correctly RLS-scoped per-user — the loop is already tenant-safe for the multi-tenant transition. Two real gaps were found and are now fixed:

1. **DOA's AI-Verify accept always tagged the correction as "title," even when only the description or starting bid changed** (`DenverLotEditor.tsx`'s `acceptVerify()` and `CreateListing.tsx`'s `handleDenverVerifyAccept()` — the lot-editor and lot-card versions, both had the identical bug). When title didn't change, this wrote a no-op `wrong_title === corrected_title` pair and never captured the real description/bid diff anywhere — eBay's equivalent path already did this correctly (checks what changed, uses the generic specifics slot). Fixed both call sites to mirror eBay's pattern: description/starting_bid changes now go into `wrongSpecifics`/`correctedSpecifics` (DOA's equivalent of eBay's item-specifics JSON slot, already existed in the schema — no migration needed), and `correctedField` is computed from what actually changed ("title" / "specifics" / "both") instead of hardcoded. Affects roughly half of the 263 `ai_verify` corrections captured to date (135 are `platform=denver`).
2. **`refine-listing`'s verify mode read the 5 most recent lessons but never called `record_lesson_injections`** — only `generate-listing`'s eBay branch did. Since `refine-listing` verify mode is what both DOA and eBay AI Verify actually call, this is the busiest lesson-consuming path, and it was invisible in `correction_injections` (3986 rows, all from the eBay-only generate-listing path). Added the identical RPC call used in generate-listing.
...
## 2026-08-25 — EstateSales auth-detection fixed: icon glyph changed, not credentials or bot detection

**Context:** Local `RUN-ESTATESALES-AGENT.bat` opened DOA and EstateSales.net correctly, transferred nothing, and landed on a "different page" — FATAL "Could not find EstateSales.net email input." `waitForEsAuth()` proves a live session by matching literal text `"+ UPLOAD"` on the sale wizard's Pictures step. EstateSales.net replaced the leading `+` with an icon-font ligature (`"add"`) sometime after 2026-08-16 — the button now renders `"add UPLOAD"`, no `+` character anywhere, so the regex went silently, permanently false. With a genuinely live session, that false negative sent the agent to `/sign-in`, which auto-redirected to the dashboard (session was valid) instead of a login form, so the email-field lookup found nothing and the run died — zero photos transferred, exactly as observed.

**Decision:** `waitForEsAuth()` now matches the upload button by accessible role + name (`getByRole('button', { name: /upload/i })`) instead of a hand-copied icon string. Confirmed live (read-only, no clicks): old regex → `false` on the real sale-wizard URL; patched check → `true` in 3.5s. Confirmed the actual upload code (`uploadLotImages`) was never affected — it targets `input[type="file"]` via `findFirst` (DOM-attached, not text-based), with a `:has-text("UPLOAD")` fallback that already matches `"add UPLOAD"` as a case-insensitive substring. No side effects from the two failed runs: both go through `test-local.js` (`AGENT_TEST_MODE=true` → ledger disabled) and died before any ledger/upload code ran.

**Why this matters going forward:** this is the second time an ES.net UI wording/icon change silently broke a *positive* auth marker (see 2026-07-02: TinyMCE/password-field detection). The `"account home"` marker in the same function is separately fragile (`.first()` can resolve a hidden mobile-nav duplicate before the visible sidebar link) but was not the cause here — it's never reached on the sale-wizard URL, which has no such text. Left as-is; not exercised in production today.

**Consequence:** `estatesales-agent/probe-auth-markers.mjs` added (untracked, read-only, same convention as `probe-doa-login.mjs`/`probe-editor-dom.mjs`) — dumps current auth-marker state + editor-page controls against a live URL without clicking or uploading anything. Run it first the next time ES.net "isn't working." Fix on branch `fix/estatesales-upload-auth-marker` @ `60be564`.
